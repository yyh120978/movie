<template>
    <div>
        <h2>
            {{movie.title}}
            <span style="color: lightblue">({{movie.publishDate}})
        </span>
        </h2>
        <el-row type="flex" justify="center" :gutter="20">
            ...
            <el-col :span="8" style="text-align: justify">
                <div>
                    导演:{{movie.director}}
                </div>
                <div>
                    编剧:<template v-for="(a,index) in movie.author">
                ...
                <div>官方网站:<a :href="movie.address" target="_blank">{{movie.address}}</a></div>
                <div>制片国家/地区:{{movie.country}}</div>
                <div>语言:{{movie.lan}}</div>
                <div>片长:{{movie.timeLength}}分钟</div>
                <div>又名:{{movie.alias}}</div>
            </el-col>
            <el-col :span="5" style="border-left: 1px solid lightblue;text-align: justify">
                <h5 style="color: lightblue;margin: 5px">豆瓣评分</h5>
                <div>
